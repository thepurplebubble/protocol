...
### Overview
The backend consists of a mesh network of REST API servers that communicate with each other to transmit client packets. Clients use separate servers for transmission (tx) and reception (rx) to protect their anonymity. Clients send messages by POSTing to the tx server and retrieve messages by GETting from the rx server. Clients also send fake GET and POST packets to help mask their identity. Servers must implement mechanisms to detect and handle misbehaving nodes that refuse to relay messages or drop packets.
...
8. **Fake Packets**: Clients send fake GET and POST packets to mask their identity.
9. **Server Behavior Monitoring**: Mechanisms to detect and handle misbehaving servers.
...
4. **Fake Packets**: Clients send fake GET and POST packets to enhance anonymity.
...
- **Multiple Location Login**: Mechanism to handle message sync across multiple login locations.
- **Voice Call Support**: Future consideration for voice calls.
